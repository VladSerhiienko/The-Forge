...
void iDecodeColorBlock(unsigned char *dest, int w, int h, int xOff, int yOff, ImageFormat::Enum format, int red, int blue, unsigned char *src)
{
...
{
  return ((format >= ImageFormat::R8S) && (format <= ImageFormat::RGBA16S)) || ((format >= ImageFormat::R16I) && (format <= ImageFormat::RGBA32I));
}

bool ImageFormat::IsStencilFormat(const ImageFormat::Enum format)
{
  return (format == ImageFormat::D24S8);
}

bool ImageFormat::IsDepthFormat(const ImageFormat::Enum format)
{
  return (format >= ImageFormat::D16 && format <= ImageFormat::D32F);
}
...
{
  if (mFormat < ImageFormat::R32F || mFormat > ImageFormat::RGBA32F) return false;
...
uint Image::GetMipMappedSize(uint w, uint h, uint d, uint startMip, uint nMipMapLevels, ImageFormat::Enum srcFormat)
{
	w >>= startMip;
	h >>= startMip;
	d >>= startMip;
...
bool Image::iLoadDDSFromMemory(const char* memory, uint32_t memSize, const bool useMipMaps, memoryAllocationFunc pAllocator, void* pUserData)
{
  DDSHeader header;
...
bool Image::iLoadSTBIFromMemory(const char *buffer, uint32_t memSize, const bool useMipmaps, memoryAllocationFunc pAllocator, void* pUserData)
{
...
bool Image::iLoadGNFFromMemory(const char* memory, size_t memSize, const bool useMipMaps)
{
  ...
  mMipMapCount = ((!useMipMaps) || (gnfContents->m_textures[0].getLastMipLevel() == 0)) ? 1 : gnfContents->m_textures[0].getLastMipLevel();
...
{
	if (ImageFormat::IsCompressedFormat(mFormat)) return false;
	if (!(mWidth) || !isPowerOf2(mHeight) || !isPowerOf2(mDepth)) return false;
...
bool Image::iSwap(const int c0, const int c1) {
  if (!ImageFormat::IsPlainFormat(mFormat)) return false;
...
bool Image::iSaveDDS(const char *fileName)
{
 ...
 // header.
  ...
  header.mCaps.mDWCaps1 = DDSCAPS_TEXTURE | (mMipMapCount > 1 ? DDSCAPS_MIPMAP | DDSCAPS_COMPLEX : 0) | (mDepth != 1 ? DDSCAPS_COMPLEX : 0);
